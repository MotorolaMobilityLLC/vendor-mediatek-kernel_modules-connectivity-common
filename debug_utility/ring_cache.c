...
unsigned int ring_cache_overwrite_prepare(unsigned int sz, struct ring_cache_segment *seg,
						      struct ring_cache *ring_cache)
{
	...
	memset(seg, 0, sizeof(struct ring_cache_segment));
	if (sz > ring_cache->max_size - (wt - rd))
		ring_cache->read += sz - (ring_cache->max_size - (wt - rd));
	seg->remain = sz;
	/* ring_cache_dump(__func__, ring_cache); */
	/* ring_cache_dump_segment(__func__, seg); */
	return seg->remain;
...
void __ring_cache_segment_prepare(unsigned int from, unsigned int sz, struct ring_cache_segment *seg,
					      struct ring_cache *ring_cache)
{
	...
	if (ring_cache_pos + sz <= ring_cache->max_size)
		seg->sz = sz;
	else
...
{
	__ring_cache_segment_prepare(from, seg->remain, seg, ring_cache);
}
...
void _ring_cache_segment_prepare_item(unsigned int from, struct ring_cache_segment *seg, struct ring_cache *ring_cache)
{
	...
	size = (seg->remain ? 1 : 0);
	__ring_cache_segment_prepare(from, size, seg, ring_cache);
}
