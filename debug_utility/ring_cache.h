...
unsigned int ring_cache_overwrite_prepare(unsigned int sz,
	struct ring_cache_segment *seg, struct ring_cache *ring_cache);
...
#define RING_CACHE_READ_FOR_EACH(_sz, _seg, _ring_cache) \
	for (ring_cache_read_prepare(_sz, &(_seg), _ring_cache), \
		_ring_cache_segment_prepare((_ring_cache)->read, &(_seg), (_ring_cache)); \
		(_seg).sz > 0; \
...
#define RING_CACHE_READ_FOR_EACH_ITEM(_sz, _seg, _ring_cache) \
	for (ring_cache_read_prepare(_sz, &(_seg), _ring_cache), \
		_ring_cache_segment_prepare_item((_ring_cache)->read, &(_seg), (_ring_cache)); \
		(_seg).sz > 0; \
		_ring_cache_read_commit(&(_seg), (_ring_cache)), _ring_cache_segment_prepare_item((_ring_cache)->read, \
			&(_seg), (_ring_cache)))

#define RING_CACHE_WRITE_FOR_EACH(_sz, _seg, _ring_cache) \
	for (ring_cache_write_prepare(_sz, &(_seg), _ring_cache),\
		_ring_cache_segment_prepare((_ring_cache)->write, &(_seg), (_ring_cache)); \
		(_seg).sz > 0; \
		_ring_cache_write_commit(&(_seg), (_ring_cache)), _ring_cache_segment_prepare((_ring_cache)->write, \
			&(_seg), (_ring_cache)))

#define RING_CACHE_OVERWRITE_FOR_EACH(_sz, _seg, _ring_cache) \
	for (ring_cache_overwrite_prepare(_sz, &(_seg), _ring_cache), \
		_ring_cache_segment_prepare((_ring_cache)->write, &(_seg), (_ring_cache)); \
		(_seg).sz > 0; \
		_ring_cache_write_commit(&(_seg), (_ring_cache)), _ring_cache_segment_prepare((_ring_cache)->write, \
			&(_seg), (_ring_cache)))
...
void _ring_cache_segment_prepare_item(unsigned int from, struct ring_cache_segment *seg, struct ring_cache *ring_cache);
